...
$execute in minecraft:overworld store success score get_datacur.ifnewentity entity_custom_data.temp run summon minecraft:falling_block 0 -10000 0 {CustomName:'"Template:entity_custom_data"',DropItem:0b,NoGravity:1b,Tags:["entity_custom_data.temp","datapacks.tmp","tmp"],Time:600s,UUID:$(d)}
# say summon:{@e[type=minecraft:falling_block,tag=entity_custom_data.temp]}
